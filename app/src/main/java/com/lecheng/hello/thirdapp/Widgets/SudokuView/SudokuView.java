package com.lecheng.hello.thirdapp.Widgets.SudokuView;
...
import com.lecheng.hello.thirdapp.Utils.DensityUtil;


//http://www.jb51.net/article/76073.htm
public class SudokuView extends View {
    ...
    private void init() {
        ...
        mCellStrokeWidth = DensityUtil.dip2px(getContext(), DEFALUT_CELL_STROKE_WIDTH);
    ...
    private void drawCell(Canvas canvas) {
        for (int i = 0; i < 9; i++) {
            canvas.drawCircle(mCells[i].getCenterX(), mCells[i].getCenterY(),
                    mCellRadius, mCells[i].isSelected() ? mPaintSelected : mPaintNormal);
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                mFinish = true;
                String strSelect = mSbSelected.toString();
                if (strSelect.length() > 7) {
//                    Toast.makeText(getContext(), strSelect, Toast.LENGTH_SHORT).show();
                    mViewClick.onClick(strSelect);
                } else Toast.makeText(getContext(), "连线的长度需要在3个以上!", Toast.LENGTH_SHORT).show();
                break;
